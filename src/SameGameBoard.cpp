...
void CSameGameBoard::SetupBoard(void)
{
	...
	// Устанавливаем каждому блоку случаный цвет
	for (int row = 0; row < m_nRows; row++)
		for (int col = 0; col < m_nColumns; col++)
...
{
	// Проверяем столбец за столбцом(слева-напрвао)
	for (int col = 0; col < m_nColumns; col++)
	{
		...
		{
			int nColor = m_arrBoard[row][col];
			// Если мы попали на ячейку с цветом фона, то это значит, что столбец уничтожен
			if (nColor == 0)
				break;
			else
			{
